...
public class BatchTransferExample {
	...
	public BatchTransfer create() {
		...
		params.put("batch_no", orderNo); // 企业转账使用的商户内部订单号。wx(新渠道)、wx_pub 规定为 1
											// ~ 50 位不能重复的数字字母组合
		params.put("amount", 2000); // 订单总金额, 人民币单位：分（如订单总金额为 1 元，此处请填
									// 100,企业付款最小发送金额为 1 元）
	 ...
	 * 根据 ID 查询
	 *
	 * 根据 ID 查询批量转账记录。 参考文档：https://www.pingxx.com/api
	 * 
	 * @param id
	 */
