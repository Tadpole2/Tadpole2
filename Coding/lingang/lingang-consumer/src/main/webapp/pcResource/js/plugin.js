(function(){
	...
	var indexDrop = $('.index-dropdown');
	if(indexDrop.length > 0){
		indexDrop.parent('li').hover(function(){
...
var check = function(){
	...
	this.radioChange = function(t){
		var _this = $(t);
		var radio = _this.find('input[type="radio"]');
		$('.check-i').removeClass('check');
		$('.check-i').find('input[type="radio"]').prop('checked',false);
		_this.addClass('check');
